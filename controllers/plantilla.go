...
func (c *PlantillaController) GetOne() {
	...
	// plantillasformated := make([]map[string]interface{}, 0)

	var alerta models.Alert

	plantilla, err := models.ConsultarPlantilla(idStr)

	fmt.Println(err)
	if err == nil {
		alerta.Type = "OK"
		alerta.Code = "200"
		alerta.Body = plantilla
	} else {
		alerta.Type = "error"
		alerta.Code = "400"
...
func (c *PlantillaController) Put() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &registroPlantilla); err == nil {
		...
		result, err1 := models.ActualizarPlantilla(idStr, registroPlantilla)
